...
| `.stack-meta`（v0.2.19） | 堆叠条行内右侧小字（**逐日分母 / 独立作者数**）：在 `.stack-row` 内追加 `<span class="stack-meta">…</span>`。扩列由 `.stack-row:has(> .stack-meta)` 触发——**没带该元素的行排版与 v0.2.18 逐像素一致**（这是"皮肤只增不改"的落地方式）；≤880px 折成单列 |
| `.ci-band`（v0.2.19） | **95% 区间带**，一个类两种用法、靠元素上下文区分：① 趋势图内 `polygon.ci-band`（与折线同色系 `.s1–.s4`，低透明度填充，**不得加描边**——会盖住折线）；② 表格/正文内 `span.ci-band` 区间徽章。**纪律**：区间必须与点估计并排出现；跨 0 的差值只能写「不得声称有差异」，只给点估计不给区间同样不合格 |
