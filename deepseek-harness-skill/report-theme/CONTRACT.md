...
- `--abstract`：ch1 摘要 md（自动识别：**结构化标签式**——标签单独成段写 `**一句话结论**` 等、后空行接内容 → `.abstract dl` 行；或旧两段式——空行分段：段一 事件→进展→舆论、段二 真实性→结构提示）；推荐标签序：一句话结论/事件定性/事实核查/平台采样/舆论主形态/少数派/事件跨度（见模板注释）；
