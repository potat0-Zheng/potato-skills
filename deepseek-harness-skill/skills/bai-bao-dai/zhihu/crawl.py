...
def fmt_time(ts):
    """epoch 秒 → 'YYYY-MM-DD HH:MM'；缺失/非法返回空串。"""
    try:
        ts = int(ts)
        if ts <= 0:
            return ""
...
def crawl_single_answer(answer_id: str, out_path: str):
    ...
    created = fmt_time(resp.get("created_time"))
...
def crawl_whole_question(question_id: str, out_path: str, limit=None):
    ...
    for offset in range(0, total, page_size):
        page_url = (
            f"https://www.zhihu.com/api/v4/questions/{question_id}/answers"
            f"?include=data%5B*%5D.content,excerpt,author,voteup_count,created_time"
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        ...
        for i, item in enumerate(all_data):
            ...
            created = fmt_time(item.get("created_time"))
            ...
            if url_token:
                f.write(f"- 主页：https://www.zhihu.com/people/{url_token}\n")
            f.write(f"- 回答链接：https://www.zhihu.com/question/{question_id}/answer/{item.get('id', '')}\n")
            f.write(f"\n{html_to_md(content)}\n\n")
            f.write("---\n\n")
