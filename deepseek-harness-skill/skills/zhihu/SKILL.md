...
# 全问题模式可选限制条数：
python "{SKILL_DIR}/crawl.py" "<知乎URL>" "<输出路径>.md" --limit 5
```

脚本会自动判断 URL 类型：
- 含 `/answer/` → 爬取单条回答
- 仅含 `/question/` → 爬取该问题下全部回答（可用 `--limit N` 只爬前 N 条，便于轻量浏览/测试）

### 4. 健壮行为（v2）

- 无 cookie 或 cookie 失效时，脚本会打印明确提示（"cookie 缺失/过期，请更新 cookie.txt"）并以非 0 退出码结束，不再报裸 traceback。
- 网络失败、非 JSON 响应（风控）同样给出可读提示。
- 若需修复爬取：提示用户更新 cookie（见下方「Cookie 更新」）。
