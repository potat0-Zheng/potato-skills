...
PIYAO_SEARCH_URL = "https://so.news.cn/xhtvapp/rumourSearch"  # 辟谣平台底层搜索 API（新华网提供）
...
class ChinaVerifyClient:
    ...
    def piyao_search(self, keyword: str) -> dict[str, Any]:
        ...
        底层为新华网 rumourSearch API（辟谣平台前端同源），返回结构化 JSON。

        Returns:
            {
                "keyword": "...",
                "found": True/False,
                "results": [{"title": "...", "url": "...", "date": "...", "summary": "...", "tag": "..."}]
            }
        """
        results: list[dict[str, str]] = []
        try:
            params = {
                "title": keyword,   # 标题搜索；全文搜索可改用 content=
                "pageNum": 1,
                "pageSize": 10,
                "timeInterval": "",
                "startTime": "",
            ...
            if data.get("code") != 200:
                logger.warning("辟谣平台搜索异常: %s", data.get("message"))
                return {"keyword": keyword, "source": "中国互联网联合辟谣平台",
                        "found": False, "results": []}
            lst = (data.get("content") or {}).get("resultList") or []
            for it in lst[:10]:
                clean_title = (it.get("title") or "").strip()
                if not clean_title:
                    continue
                results.append({
                    "title": clean_title,
                    "url": it.get("url") or "",
                    "date": it.get("publishTime") or it.get("createDate") or "",
