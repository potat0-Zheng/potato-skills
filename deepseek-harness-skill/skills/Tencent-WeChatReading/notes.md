...
**排序规则：** `reviews` 数组按 `createTime` **降序**排列（最新在前，最早在后）。用户说"前三条""最早几条"时，必须拉取全部数据后按 `createTime` 升序取最前面的条目，不能直接把第一页前三条当作"前三条"。

**拉取全部数据的正确方式：** `count` 较小时（如默认 20），`hasMore` 字段可能不可靠——即使 `hasMore=0`，实际可能仍有未返回的数据。**推荐做法**：第一次请求就传一个足够大的 `count`（如 200），然后验证 `len(reviews) == totalCount`；不相等时用返回的 `synckey` 继续翻页直到拿满全部条目。
...
2. **有 bookId 或书名，问单本书笔记内容**：同时调 `/book/bookmarklist`（划线内容）和 `/review/list/mine`（想法/点评内容），合并展示当前可导出的笔记内容。
   - **"最新/最近"笔记**：`/review/list/mine` 默认返回最新在前，取首页前几条即可。
   - **"最早/前几条"笔记**：必须拉取全部数据（传大 `count` 并验证 `len(reviews) == totalCount`，不等则用 `synckey` 翻页），再按 `createTime` 升序取最早的条目。不能直接把返回数组的前几条当最早——返回是倒序的。
